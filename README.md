...
项目界面与素材为原创实现，使用应用内全屏阻挡，不等同于 Windows 登录锁屏或不可绕过的安全边界；不修改系统安全策略，不阻止系统安全操作或任务管理器。终止进程、重启后不会继续强制休息，未完成休息不结果。不包含系统屏幕滤蓝光、自动开机启动或云同步。当前交付为未签名的 Windows x64 便携版。
...
开始专注后默认按“专注 → 休息 → 下一轮专注”连续运行，包含每四轮的长休息，无需反复点击开始。旧设置首次加载会升级到连续默认值；之后可在设置中关闭自动开始下一轮，后续启动会尊重这一选择。

休息场景“无”（1.3.2 修正）：不强制全屏、不拦截快捷键，普通提醒可直接关闭，关闭后计时继续循环。已保存的强制休息开关不会覆盖“无”的选择。其他场景仍遵循强制休息开关。果子仍只统计完整强制休息。
