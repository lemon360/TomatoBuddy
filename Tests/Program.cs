...
var optionalRest = new Preferences { RestScene = "None", StrictRest = true };
Check(!optionalRest.EffectiveStrictRest, "None overrides existing strict setting");
Check(optionalRest.AutoStartAfter(Phase.Focus) && optionalRest.AutoStartAfter(Phase.ShortBreak), "None keeps continuous timing");
optionalRest.AutoBreak = false;
Check(!optionalRest.AutoStartAfter(Phase.Focus), "None respects manual rest start preference");
optionalRest.RestScene = "Plant";
Check(optionalRest.EffectiveStrictRest, "other scenes retain strict preference");
optionalRest.StrictRest = false;
Check(!optionalRest.EffectiveStrictRest, "other scenes respect strict opt-out");
Console.WriteLine($"Total: {checks} checks passed");
