...
{
 public static void UpgradeContinuity(AppData data)
 {
  if (data.SettingsVersion >= 1) return;
  data.Settings.AutoFocus = true; data.Settings.AutoBreak = true; data.SettingsVersion = 1;
 }
 ...
 public AppData Load()
 {
  ...
  if (!File.Exists(path)) return new() { SettingsVersion = 1 };
  try
  {
